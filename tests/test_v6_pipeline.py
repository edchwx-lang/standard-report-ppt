...
class V6PipelineTests(unittest.TestCase):
    ...
    def run_with_fakes(
        ...
        *,
        ...
        def loader(name, path):
            ...
            if "v623_deconstruction_output_contract" in name:
                return SimpleNamespace(
                    audit_deconstruction_output=lambda pptx, template: {
    ...
    def test_first_deconstruct_success_is_reused_without_a_second_build(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual("6.2.3", result["skill_version"])
            self.assertTrue(Path(result["deconstruction_output_contract"]).is_file())
            self.assertTrue(
