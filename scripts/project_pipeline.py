        ...
        required = [
            ...
            *[
                ...
                for name in (
                    ...
                    "v61_deconstruction_acceptance.py",
                    "v623_deconstruction_output_contract.py",
                    "v622_prompt_guard.py",
...
def _run_v6_project(
    ...
    *,
    ...
    if mode == "deconstruct":
        output_contract_module = guarded(
            "deconstruction_output_contract",
            lambda: _load_module(
                "standard_report_v623_deconstruction_output_contract",
                Path(__file__).with_name("v623_deconstruction_output_contract.py"),
            ),
        )
        output_contract = guarded(
            "deconstruction_output_contract",
            lambda: output_contract_module.audit_deconstruction_output(
                output,
                Path(__file__).resolve().parents[1] / "assets" / "company_template.pptx",
            ),
        )
        output_contract_path = project_dir / ".build" / "deconstruction_output_contract.json"
        guarded(
            "deconstruction_output_contract",
            lambda: _write_json_atomic(output_contract_path, output_contract),
        )
        ...
        acceptance = guarded(
            ...
            lambda: acceptance_module.evaluate_deconstruction_acceptance(
                ...
                output_contract_report_sha256=_sha256_file(output_contract_path),
            ),
        )
        ...
        if not acceptance.get("accepted"):
            guarded(
                ...
                lambda: (_ for _ in ()).throw(
                    ValueError(
                        "V6.2.3 deconstruction acceptance failed: "
                        + "; ".join(
                            str(item.get("message", item))
    ...
    result = {
        "schema_version": "6.0",
        "pipeline_revision": "6.0.0",
        "skill_version": "6.2.3" if mode == "deconstruct" else "6.0.1",
        ...
        "deconstruction_output_contract": (
            str(output_contract_path) if output_contract_path is not None else None
        ),
    }
    if bitmap_release is not None:
        acceptance = guarded(
